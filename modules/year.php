...
while ($month_indice < $month_end)
{
    ...
    echo "</caption>";
	$sql = "select room_name, capacity, r.id, description from ".TABLE_PREFIX."_room r INNER JOIN ".TABLE_PREFIX."_area a ON r.area_id = a.id where area_id=$area AND (r.id IN (SELECT id_room FROM ".TABLE_PREFIX."_show_room WHERE login = '".getUserName()."') OR a.access <> 'r') order by r.order_display,room_name";
	$res = grr_sql_query($sql);
