...
function make_area_select_html( $link, $current_site, $current_area, $year, $month, $day, $user)
{
	...
	if ($use_multi_site == 'y')
	{
		...
		if ($current_site != -1)
			$sql = "SELECT a.id, a.area_name,a.access FROM ".TABLE_PREFIX."_area a, ".TABLE_PREFIX."_j_site_area j WHERE a.id=j.id_area and j.id_site=$current_site AND a.id IN (SELECT id_area from ".TABLE_PREFIX."_show_room WHERE login = '".$user."') ORDER BY a.order_display, a.area_name";
		else
			$sql = "";
	}
	else
		//$sql = "SELECT id, area_name,access FROM ".TABLE_PREFIX."_area WHERE id IN (SELECT id_area from ".TABLE_PREFIX."_show_room WHERE login = '".$user."') ORDER BY order_display, area_name";
	...
	if (Settings::get("module_multisite") == "Oui")
	{
		...
		if ($res)
		{
			for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
			{
				...
				$sql = "SELECT id_area
				FROM ".TABLE_PREFIX."_j_site_area
				WHERE ".TABLE_PREFIX."_j_site_area.id_site='".$row[0]."' AND id IN (SELECT id_area FROM ".TABLE_PREFIX."_show_room WHERE login = '".$user."')";
				$res2 = grr_sql_query($sql);
...
function make_area_item_html( $link, $current_site, $current_area, $year, $month, $day, $user)
{
	...
	if ($use_multi_site == 'y')
	{
		...
		if ($current_site != -1)
			$sql = "SELECT a.id, a.area_name,a.access
		FROM ".TABLE_PREFIX."_area a, ".TABLE_PREFIX."_j_site_area j 
		WHERE a.id=j.id_area and j.id_site=$current_site AND (a.id IN (SELECT id_area FROM ".TABLE_PREFIX."_show_room WHERE login = '".$user."') OR access <> 'r') 
		ORDER BY a.order_display, a.area_name";
		else
			$sql = "";
	}
	else
	{
		$sql = "SELECT id, area_name,access
		FROM ".TABLE_PREFIX."_area
		WHERE id IN (SELECT id_area FROM ".TABLE_PREFIX."_show_room WHERE login = '".$user."')
		ORDER BY order_display, area_name";
	}
...
function make_room_item_html($link, $current_area, $current_room, $year, $month, $day, $user)
{
	...
	$sql = "SELECT r.id, r.room_name, description 
	FROM ".TABLE_PREFIX."_room r INNER JOIN ".TABLE_PREFIX."_area a ON r.area_id = a.id
	WHERE r.area_id='".protect_data_sql($current_area)."' 
	AND (r.id IN (SELECT id_room FROM ".TABLE_PREFIX."_show_room WHERE login = '".$user."') 
	OR a.access <> 'r')
	ORDER BY r.order_display, r.room_name";
	$res = grr_sql_query($sql);
	if ($res)
	{
...
function pageHeader2($day = '', $month = '', $year = '', $type_session = 'with_session', $adm=0)
{
	...
	if ((!isset($_GET['pview'])) || ($_GET['pview'] != 1))
	{
		...
		if (!(isset($desactive_bandeau_sup) && ($desactive_bandeau_sup == 1) && ($type_session != 'with_session')))
		{
			...
			//Accueil
			echo '<div class="accueil ">',PHP_EOL,'<h2>',PHP_EOL,'<a href="'.$racine.page_accueil('yes'),'day=',$day,'&amp;year=',$year,'&amp;month=',$month,'">',Settings::get("company"),'</a>',PHP_EOL,'</h2>',PHP_EOL, Settings::get('message_accueil'),'<p>Pour voir plus de salles, demandez les droits à l\'administrateur du domaine souhaité.</p></div>',PHP_EOL;
