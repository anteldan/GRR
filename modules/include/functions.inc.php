...
function pageHeader2($day = '', $month = '', $year = '', $type_session = 'with_session', $adm=0)
{
	...
	if ((!isset($_GET['pview'])) || ($_GET['pview'] != 1))
	{
		...
		if (!(isset($desactive_bandeau_sup) && ($desactive_bandeau_sup == 1) && ($type_session != 'with_session')))
		{
			...
			//Accueil
			echo '<div class="accueil ">',PHP_EOL,'<h2>',PHP_EOL,'<a href="'.$racine.page_accueil('yes'),'day=',$day,'&amp;year=',$year,'&amp;month=',$month,'">',Settings::get("company"),'</a>',PHP_EOL,'</h2>',PHP_EOL, Settings::get('message_accueil'),'<p>Pour voir plus de salle, demandé les droits au gestionnaire du domaines souhaité.</p></div>',PHP_EOL;
