...
$sql = "SELECT room_name, capacity, r.id, description, statut_room, show_fic_room, delais_option_reservation, moderate FROM ".TABLE_PREFIX."_room r INNER JOIN ".TABLE_PREFIX."_area a ON r.area_id = a.id WHERE area_id='".protect_data_sql($area)."' AND (r.id IN (SELECT id_room FROM ".TABLE_PREFIX."_show_room WHERE login = '".getUserName()."') OR a.access <> 'r') ORDER BY r.order_display, room_name";
...
//row[11] = Type_name
$sql = "SELECT start_time, end_time, ".TABLE_PREFIX."_entry.id, name, beneficiaire, room_name, ".TABLE_PREFIX."_entry.description, type, ".TABLE_PREFIX."_entry.moderate, beneficiaire_ext, r.id, ".TABLE_PREFIX."_type_area.type_name 
FROM (".TABLE_PREFIX."_entry inner join ".TABLE_PREFIX."_room r on ".TABLE_PREFIX."_entry.room_id=r.id INNER JOIN ".TABLE_PREFIX."_area a ON r.area_id = a.id ) 
...
AND (r.id IN (SELECT id_room FROM ".TABLE_PREFIX."_show_room WHERE login = '".getUserName()."') OR a.access <> 'r')
ORDER by start_time, end_time, r.room_name";
...
<?php
include "include/hide_reservation.php";
?>