...
$sql = "SELECT area_name, room_name, capacity, description, statut_room, moderate, delais_option_reservation, comment_room, show_comment FROM ".TABLE_PREFIX."_room r INNER JOIN ".TABLE_PREFIX."_area a ON r.area_id = a.id WHERE r.id=$room AND (r.id IN (SELECT id_room FROM ".TABLE_PREFIX."_show_room WHERE login = '".getUserName()."') OR a.access <> 'r') ORDER BY r.order_display, room_name";
...
$row = grr_sql_row($ressources, 0);
$this_area_name = $row['0'];
$this_room_name = $row['1'];
$this_room_max = $row['2'];
$this_room_name_des = $row['3'];
$this_statut_room = $row['4'];
$this_moderate_room = $row['5'];
$this_delais_option_reservation = $row['6'];
$this_area_comment = $row['7'];
$this_area_show_comment = $row['8'];
